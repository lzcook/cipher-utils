import com.lzc.lib.util.cipher.asymmetry.SM2Utils;
...
/**
 * SM2Utils 测试类
 * 包含基本加解密、签名验证、密钥导出导入（DER和PEM格式）等功能测试
 *
 * @author lzc
 * 2019/2/21 20:56
 */
public class SM2Test {
    ...
    public void testExportPublicKeyToDER() {
        ...
        byte[] derBytes = SM2Utils.exportPublicKeyToDER(publicKeyBase64);
    ...
    public void testExportPrivateKeyToDER() {
        ...
        byte[] derBytes = SM2Utils.exportPrivateKeyToDER(privateKeyBase64);
    ...
    public void testExportPublicKeyToPEM() {
        ...
        String pemString = SM2Utils.exportPublicKeyToPEM(publicKeyBase64);
    ...
    public void testExportPrivateKeyToPEM() {
        ...
        String pemString = SM2Utils.exportPrivateKeyToPEM(privateKeyBase64);
    ...
    public void testImportPublicKeyFromDER() {
        ...
        // 从 DER 导入
        ECPublicKey importedKey = SM2Utils.importPublicKeyFromDER(derBytes);
    ...
    public void testImportPrivateKeyFromDER() {
        ...
        // 从 DER 导入
        ECPrivateKey importedKey = SM2Utils.importPrivateKeyFromDER(derBytes);
    ...
    public void testImportPublicKeyFromPEM() {
        ...
        // 从 PEM 导入
        ECPublicKey importedKey = SM2Utils.importPublicKeyFromPEM(pemString);
        Assert.assertNotNull("导入的公钥不应为空", importedKey);
    ...
    public void testImportPrivateKeyFromPEM() {
        ...
        // 从 PEM 导入
        ECPrivateKey importedKey = SM2Utils.importPrivateKeyFromPEM(pemString);
        Assert.assertNotNull("导入的私钥不应为空", importedKey);
    ...
    public void testPublicKeyDERFile() {
        ...
        try {
            // 导出到文件
            SM2Utils.exportPublicKeyToDERFile(publicKeyBase64, filePath);
            System.out.println("已导出公钥到文件: " + filePath);

            // 从文件导入
            ECPublicKey importedKey = SM2Utils.importPublicKeyFromDERFile(filePath);
            Assert.assertNotNull("导入的公钥不应为空", importedKey);

            // 验证
            byte[] original = SM2Utils.exportPublicKeyToDER(publicKeyBase64);
    ...
    public void testPrivateKeyDERFile() {
        ...
        try {
            // 导出到文件
            SM2Utils.exportPrivateKeyToDERFile(privateKeyBase64, filePath);
            System.out.println("已导出私钥到文件: " + filePath);

            // 从文件导入
            ECPrivateKey importedKey = SM2Utils.importPrivateKeyFromDERFile(filePath);
            Assert.assertNotNull("导入的私钥不应为空", importedKey);

            // 验证
            byte[] original = SM2Utils.exportPrivateKeyToDER(privateKeyBase64);
            byte[] imported = importedKey.getEncoded();
    ...
    public void testPublicKeyPEMFile() {
        ...
        try {
            // 导出到文件
            SM2Utils.exportPublicKeyToPEMFile(publicKeyBase64, filePath);
            System.out.println("已导出公钥到文件: " + filePath);

            // 从文件导入
            ECPublicKey importedKey = SM2Utils.importPublicKeyFromPEMFile(filePath);
            Assert.assertNotNull("导入的公钥不应为空", importedKey);
    ...
    public void testPrivateKeyPEMFile() {
        ...
        try {
            // 导出到文件
            SM2Utils.exportPrivateKeyToPEMFile(privateKeyBase64, filePath);
            System.out.println("已导出私钥到文件: " + filePath);

            // 从文件导入
            ECPrivateKey importedKey = SM2Utils.importPrivateKeyFromPEMFile(filePath);
            Assert.assertNotNull("导入的私钥不应为空", importedKey);
    ...
    public void testDERAndPEMConversion() {
        ...
        // 公钥：DER -> PEM -> DER
        byte[] publicDER1 = SM2Utils.exportPublicKeyToDER(publicKeyBase64);
        String publicPEM = SM2Utils.exportPublicKeyToPEM(publicKeyBase64);
        ECPublicKey publicKey = SM2Utils.importPublicKeyFromPEM(publicPEM);
        ...
        // 私钥：DER -> PEM -> DER
        byte[] privateDER1 = SM2Utils.exportPrivateKeyToDER(privateKeyBase64);
        String privatePEM = SM2Utils.exportPrivateKeyToPEM(privateKeyBase64);
        ECPrivateKey privateKey = SM2Utils.importPrivateKeyFromPEM(privatePEM);
