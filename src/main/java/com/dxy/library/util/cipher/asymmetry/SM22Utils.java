 ...
 *
 * SM2 基于椭圆曲线密码学（ECC），使用 PKCS#8 格式存储私钥，X.509 格式存储公钥
 *
 * @author duanxinyuan
 * 2025/11/03
 */
public class SM22Utils {
    ...
    // EC PKCS#1 格式标记（可选，OpenSSL 的 SEC1 格式）
    ...
    public static byte[] exportPublicKeyToDER(String publicKey) {
        ECPublicKey ecPublicKey = SM2Utils.getPublicKey(publicKey);
    ...
    public static byte[] exportPrivateKeyToDER(String privateKey) {
        ECPrivateKey ecPrivateKey = SM2Utils.getPrivateKey(privateKey);
    ...
    public static ECPublicKey importPublicKeyFromDER(byte[] derBytes) {
        return SM2Utils.getPublicKey(derBytes);
    }
    ...
    public static ECPrivateKey importPrivateKeyFromDER(byte[] derBytes) {
        return SM2Utils.getPrivateKey(derBytes);
    }
